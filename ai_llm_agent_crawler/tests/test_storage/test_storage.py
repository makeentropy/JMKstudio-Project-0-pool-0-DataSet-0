...
class TestStorageMonitor:
    ...
    def test_monitor_register_backend(self, monitor, backend):
        ...
        backends = monitor.get_all_backends()
        assert any(b.config.name == "monitor_test" for b in backends)
    ...
    def test_monitor_unregister_backend(self, monitor, backend):
        ...
        backends = monitor.get_all_backends()
        assert not any(b.config.name == "monitor_test" for b in backends)
    ...
    def test_monitor_get_status(self, monitor, backend):
        ...
        assert "monitor_test" in status["connected_backends"]
...
class TestIntegration:
    ...
    def test_full_workflow(self, temp_dir):
        ...
        assert pool.write("test3.txt", b"test data 3").success  # 不使用标签进行写入
